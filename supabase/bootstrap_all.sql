...
-- ║                                                                            ║
-- ║ NON è una migration: è la concatenazione, NELL'ORDINE CORRETTO, dei file    ║
-- ║ in supabase/migrations/. Comodo per applicare tutto in un colpo solo dal    ║
-- ║ SQL Editor di Supabase (Dashboard → SQL Editor → New query → incolla → Run).║
-- ║ Idempotente: si può rilanciare senza danni (if not exists / on conflict /   ║
-- ║ create or replace). La via canonica resta comunque `pnpm db:push`.          ║
-- ║                                                                            ║
-- ║ Dopo l'esecuzione, in Dashboard:                                           ║
-- ║  • Authentication → Providers → Anonymous sign-ins = ON                    ║
-- ║  • Realtime → disattiva "Allow public access" (channel privati via RLS)     ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
